class RubyLipsum
  ...
  def self.words(total = 1)
    total = 1 if total.to_i < 1

    index = total - 1
    @words ||= {}

    @words[index] ||= TEXT.gsub('.', '').gsub(',', '').split(' ')[0..index].join(' ')
  end

  def self.sentences(total = 1)
    total = 1 if total.to_i < 1

    index = total - 1
    @sentences ||= {}

    @sentences[index] ||= TEXT.split('.')[0..index].map{|sentence| sentence.strip }.join('. ')
  end

  def self.paragraphs(total = 1)
    total = total.to_i
    return TEXT if total <= 1
